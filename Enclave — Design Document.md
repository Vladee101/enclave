...
| FR7 | Фоновый приём: извлечение текста → нарезка → эмбеддинги → запись `chunks` + `chunk_embeddings`; `documents.status` `pending` → `ready` / `failed` | готово для PDF с текстовым слоем, DOCX, TXT, MD (ADR-0019); сканы — нет, OCR не поддержан |
