...
| Команда | Пул / роль | Контекст RLS | Назначение |
| --- | --- | --- | --- |
| `cmd_query {query, top_k?}` | app | да | UC9 без стриминга → `{answer, sources[], calculation}`; `calculation` — текст расчёта по таблице или `null` (ADR-0022) |
...
| Unit: расчёты по таблицам | типы столбцов по большинству ячеек, числа и даты, набранные текстом; схема плана по столбцам таблицы, закрытые столбцы; отказ от планов, не подходящих таблице; параметризация SQL; пара равенств как диапазон | `cargo test --lib tables` | есть |
| Integration: расчёты | точные суммы (десятичные), счёт, группы, диапазоны дат, числа сравниваются как числа; чужой департамент не видит ни таблицы, ни строк; удаление документа удаляет таблицы | `tests/table_aggregation.rs`; падает без политики RLS на `sheet_rows` и без триггера (проверено) | есть |
