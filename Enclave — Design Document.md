...
| 11 | Любой член департамента может напрямую удалить или изменить назначения LoRA-адаптеров своего департамента (политики `adapters_dept_*` из 004) | `migrations/004_rls.sql` | Открыто. Приложение этим путём не пользуется — адаптеры назначает администратор через `admin_pool`, — но право у `app_user` есть. Закрыть так же, как запись в `chunks` в миграции 009: отозвать привилегии |
