...
| FR18 | Таблица индексируется построчно: каждая строка — запись с листом, номером строки и заголовками столбцов; на вопрос о конкретной строке (номер договора, фамилия) находится эта строка (ADR-0020) | готово; вопросы-агрегации («сколько», «общая сумма») — нет |
