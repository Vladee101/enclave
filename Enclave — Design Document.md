...
Источник истины — `enclave/migrations/001–014`. `db/schema.sql` — исходный референсный проект, нигде не применяется и с живой схемой расходится.
...
| `cmd_query {query, top_k?, plan?}` | app | да | UC9 без стриминга → `{answer, sources[], calculation, clarification}`; `calculation` — текст расчёта по таблице или `null` (ADR-0022); `clarification` — `{question, table_id, options[{label, plan}]}` вместо ответа; `plan` — `{table_id, plan}` выбранного варианта, проверяется как вывод модели (ADR-0023) |
...
| Integration: ранжирование | 1 500 строк с общими словами и одна с нужным номером; поиск приложения под RLS находит её, чужой департамент — ничего | `tests/retrieval_ranking.rs`; без IDF падает (проверено) | есть |
