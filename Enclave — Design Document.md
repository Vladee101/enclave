...
| FR8 | Опрос статуса задачи приёма из интерфейса | код есть, фактически не работает (см. «Известные расхождения», №3) |
...
| FR14 | Журнал аудита: вход, загрузка, запрос | **нет** — таблица `audit_log` есть, не пишется |
...
| NFR4 | Задержка ответа | На эталонном железе (RTX 4050 Laptop 6 GB, Qwen 2.5 7B Q4): до первого токена ≤ 2 сек (p95), полный ответ ≤ 30 сек при `n_predict = 768`. *Целевые значения, не замерены* |
| NFR5 | Отзывчивость загрузки | `upload_document` возвращает управление ≤ 1 сек для файла ≤ 10 МБ; тяжёлая работа — только в воркере |
| NFR6 | Пропускная способность приёма | Текстовый файл 1 МБ индексируется ≤ 2 мин на эталонном железе. *Целевое, не замерено* — расчёт ниже |
...
| NFR9 | Пул соединений не блокируется моделью | Ни одна транзакция БД не открыта во время HTTP-вызова к модели или эмбеддеру (инвариант №4 CLAUDE.md). *Сейчас нарушено в приёме*, см. «Известные расхождения», №2 |
...
| NFR12 | Аппаратный минимум | 16 ГБ RAM, GPU 6 ГБ VRAM (CUDA) для 7B Q4 с частичной выгрузкой слоёв; без GPU — работоспособно, но NFR4 не выполняется |

**Бюджет задержки вопроса (NFR4)**

Порог — сумма шагов; без разбивки его нельзя ни спроектировать, ни проверить. Все значения — оценки для эталонного железа, замера ещё не было.

| Шаг | Бюджет (p95) | Примечание |
| --- | --- | --- |
| Эмбеддинг вопроса | ≤ 0.05 сек | отдельный sidecar на 8081, до открытия транзакции (инвариант №4) |
...
| Prefill промпта (~1 000 токенов: 5 фрагментов по 512 символов + инструкция) | ≤ 1.5 сек | подмена LoRA на запрос добавляет накладные расходы, не измерены |
| **До первого токена** | **≤ 1.7 сек** | запас 0.3 сек до NFR4 |
| Генерация до 768 токенов | ≤ 28 сек | ~30 ток/с для 7B Q4 при частичной выгрузке в RAM |
...
**UC10 — журнал аудита** *(не реализовано)*

- Given пользователь вошёл, загрузил документ и задал вопрос, When открывается его журнал, Then видны три события с временем, департаментом и идентификаторами документов / фрагментов
...
sequenceDiagram
  ...
  UI->>Core: cmd_query_stream(request_id, user_id, query)
  ...
  Core->>DB: лексическая ветка: content_tsv @@ plainto_tsquery LIMIT 2k
...
sequenceDiagram
  ...
  UI->>Core: cmd_upload_document(user_id, department_id, bytes)
  Core->>Core: sha-256
  Core->>FS: write (до вставки в БД)
  Core->>DB: BEGIN; set_config; INSERT documents (pending); INSERT ingestion_jobs (queued); COMMIT
  ...
  loop на каждый фрагмент
    W->>Emb: POST /embeddings
    W->>DB: INSERT chunks, chunk_embeddings (department_id родителя)
  end
  W->>DB: documents.status = ready; job = succeeded
```

Диаграмма показывает *текущий* код: цикл эмбеддингов идёт внутри открытой транзакции, что нарушает инвариант №4 (см. «Известные расхождения», №2). Целевой порядок — сначала все эмбеддинги в памяти, затем одна короткая транзакция на запись.
...
Сейчас `rls_validation` покрывает инварианты 2 и 3 и частично 1 (чужой `user_id` → ноль строк). Инварианты 4–6 в тестах не выражены.
...
| Конфигурация FTS | `english` | миграция 002 |
...
| audit_log | Журнал событий. Схема и политики есть, код в неё не пишет (FR14) |
...
| `cmd_login {user_id, pin}` | admin | — | Проверка PIN → `{ok, user_id, username, is_admin}` |
| `cmd_logout` | — | — | No-op: состояние сессии живёт только во фронтенде |

**Документы**

| Команда | Пул / роль | Контекст RLS | Назначение |
| --- | --- | --- | --- |
| `cmd_upload_document {user_id, department_id, filename, mime_type?, file_contents}` | app | да | UC6 → `JobStatus {job_id, document_id, status}`. Повторная загрузка — существующий документ и его последняя задача (FR15) |
| `cmd_list_documents {user_id}` | app | да | Документы своих департаментов |
| `cmd_list_my_departments {user_id}` | app | да | Свои департаменты — для выбора при загрузке |
| `cmd_get_job_status {job_id}` | app | **нет** | Статус задачи → `JobStatus \| null`. Без контекста политика закрыта — см. «Известные расхождения», №3 |

**Вопрос**

| Команда | Пул / роль | Контекст RLS | Назначение |
| --- | --- | --- | --- |
| `cmd_query {user_id, query, top_k?}` | app | да | UC9 без стриминга → `{answer, sources[]}` |
| `cmd_query_stream {request_id, args}` | app | да | UC9 со стримингом: токены в событии `llm-token:<request_id>` как `{token}`, итог — `{answer, sources[]}` |

`sources[]` — `{document_id, filename, excerpt (200 символов), score (RRF)}`.

**Администрирование** (все — `require_admin`, затем `admin_pool`)

| Команда | Назначение |
| --- | --- |
| `cmd_list_departments {requesting_user_id}` | Все департаменты организации |
| `cmd_create_department {requesting_user_id, name}` | UC3 |
| `cmd_list_adapters {requesting_user_id}` | Все назначения адаптеров |
| `cmd_add_adapter {requesting_user_id, department_id, adapter_path, scale}` | UC5: upsert в `adapters` по sha-256 файла + upsert назначения |
| `cmd_list_memberships {requesting_user_id}` | Все членства: пользователь × департамент |
| `cmd_add_member {requesting_user_id, user_id, department_id}` | UC4, идемпотентно (`ON CONFLICT DO NOTHING`) |
| `cmd_remove_member {requesting_user_id, user_id, department_id}` | UC4, действует со следующей транзакции участника |
...
**Идентичность вызывающего.** `user_id` передаётся интерфейсом в каждой команде, а ядро ему верит: после `cmd_login` в Rust-состоянии ничего не сохраняется. RLS гарантирует, что `user_id` не увидит чужих департаментов, но не гарантирует, что `user_id` — действительно тот, кто вошёл. Для однопользовательского десктопа с доверенным webview (без удалённого контента) это приемлемая граница доверия v1, но она должна быть записана, а не подразумеваться. Целевое решение — сессия в состоянии ядра: `cmd_login` сохраняет `user_id`, команды берут его оттуда и перестают принимать от интерфейса. Это кандидат в отдельный ADR.
...
| Unit: нарезка | границы, перекрытие, многобайтовые символы, пустой текст | `split_into_chunks` не теряет и не дублирует символы, кроме перекрытия | нет |
...
| Integration: инвариант №4 | ни одна транзакция не открыта во время HTTP-вызова | тест с медленным моковым sidecar'ом и пулом из одного соединения: параллельный запрос не ждёт | нет — и сейчас бы упал |
...
| --- | --- | --- | --- |
| 1 | Команды вопроса принимают `State<'_, LlmClient>`, а ядро регистрирует `Option<LlmClient>` | `commands/query.rs:99`, `:128`; `lib.rs:53` | Tauri ищет состояние по точному типу: `cmd_query` и `cmd_query_stream` вернут «state not managed» **даже с настоящей моделью**. Чат не заработает после установки sidecar'а, пока это не исправлено |
| 2 | Цикл эмбеддингов идёт внутри открытой транзакции | `ingest/mod.rs:80–106` | Нарушение инварианта №4, которое README и CLAUDE.md описывают как соблюдённое. Соединение занято на всё время приёма (минуты на большой файл) |
| 3 | `cmd_get_job_status` читает `ingestion_jobs` через `app_pool` без `set_config` | `commands/documents.rs:131–143` | Политика закрыта по умолчанию → всегда `null` → опрос в интерфейсе крутится вечно. Вероятная первопричина «залипшего бейджа PENDING» из HANDOFF, который там объяснён иначе |
...
| 5 | `audit_log` не пишется | весь `src-tauri/src` | FR14 не выполнен |
| 6 | `user_id` приходит от интерфейса | все команды | см. «API-контракты», «Идентичность вызывающего» |
...
- Настоящий `llama-server` (CUDA) и веса: чат 7B Q4 + модель эмбеддингов 768
- Исправить расхождения 1–3 — без них вопрос и статус приёма не работают вживую
...
- Журнал аудита (FR14 / UC10)
- Сессия в ядре вместо `user_id` от интерфейса; модель ошибок с кодами
...
| Модель не помещается в 6 ГБ VRAM вместе с эмбеддером, задержка выходит за NFR4 | Средняя | Среднее | `--n-gpu-layers` с выгрузкой в RAM; меньшая квантизация или модель 3B как запасной вариант; замер до обещаний |
...
| Подмена `user_id` из webview | Низкая для v1 | Высокое | Webview не загружает удалённый контент; сессия в ядре — v1.1 |
