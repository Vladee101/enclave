...
| FR3 | Новый профиль получает членство только в общем департаменте General; остальное членство назначает администратор (ADR-0016) | готово |
...
| FR7 | Фоновый приём: извлечение текста → нарезка → эмбеддинги → запись `chunks` + `chunk_embeddings`; `documents.status` `pending` → `ready` / `failed` | готово для PDF с текстовым слоем, DOCX, XLSX / XLSM / XLSB / XLS / ODS, TXT, MD (ADR-0019, ADR-0020); сканы — нет, OCR не поддержан |
| FR8 | Опрос статуса задачи приёма из интерфейса | готово |
| FR9 | Список документов своих департаментов | готово |
| FR10 | Вопрос → гибридный поиск по документам своих департаментов → ответ с нумерованными цитатами (`sources`) | готово, проверено на живой модели (Qwen 2.5 3B) |
| FR11 | Стриминг ответа по токенам | готово, проверено на живой модели |
...
| FR16 | Удаление документа автором или администратором: текст (фрагменты и векторы) стирается сразу, строка документа остаётся надгробием для журнала аудита (ADR-0015) | готово |
| FR17 | Администратор удаляет департамент вместе с документами: документы стираются как в FR16, членство и назначения адаптеров удаляются, департамент остаётся надгробием; General удалить нельзя (ADR-0017) | готово |
| FR18 | Таблица индексируется построчно: каждая строка — запись с листом, номером строки и заголовками столбцов; на вопрос о конкретной строке (номер договора, фамилия) находится эта строка (ADR-0020) | готово |
| FR19 | Вопрос-расчёт по таблице («сколько», «общая сумма», «среднее», «по статусам», «по годам»): модель строит план в границах JSON-схемы из столбцов таблицы, ядро выполняет его параметризованным агрегатом под RLS, под ответом показано, что посчитано; без плана — обычный поиск (ADR-0022) | готово; девять контрольных вопросов на Qwen 2.5 3B — числа верные |
| FR20 | Если план расчёта опирается на столбец, которого вопрос не называет, пользователь выбирает столбец кнопкой, и выбранный план проверяется как вывод модели; ответ с разбивкой на группы пишет ядро (ADR-0023) | готово |
| FR22 | Клик по документу в панели ограничивает вопрос выбранными файлами (чипы над полем ввода, снимаются по ×): поиск и кандидаты для расчётов — только из них; невидимый пользователю id не находит ничего | готово |
| FR21 | В чате слева — документы пользователя с фильтром по имени; у таблиц — листы и столбцы с типами; клик по столбцу вставляет его название в вопрос. Только названия и типы, без значений ячеек | готово |
...
| NFR3 | Минимальные привилегии | Путь запросов — роль без `SUPERUSER` и `BYPASSRLS`; `BYPASSRLS` есть только у `ingest_worker`, и только воркер им пользуется; у `ingest_worker` нет DDL, `CREATEDB`, `CREATEROLE`. `app_user` пишет только `documents` (вставка; `status`, `updated_at`), `ingestion_jobs` (вставка) и `audit_log` (вставка своих строк) — всё остальное только читает; новые таблицы не получают права на запись автоматически (ADR-0018) |
...
| NFR6 | Пропускная способность приёма | Текстовый файл 1 МБ индексируется ≤ 2 мин на эталонном железе. *Замер (ADR-0020): эмбеддинги 146–148 фрагм./с пакетами по 32, запись ~4,7 мс на фрагмент (почти всё — HNSW); таблица 50 000 строк — 9,5 мин (было 16,5); текст 1 МБ (~2 300 фрагментов) ≈ 25 с* |
...
**Пропускная способность приёма (NFR6).** Два этапа, замерены на таблице из 50 000 строк (ADR-0020). *Эмбеддинги* — пакетами по 32 текста на запрос, 146–148 текстов/с; один текст на запрос давал 49, а подробный лог llama-server через stdout-канал приложения срезал пакетный режим до 95, поэтому серверы запускаются с `--log-verbosity 1`. *Запись* — одной транзакцией (инвариант №4), по 1 000 фрагментов на `INSERT … UNNEST`; ~4,7 мс на фрагмент, из них ~3,7 мс — обслуживание общего HNSW-индекса, неустранимое без его перестройки. Предел — 100 000 фрагментов на документ (~11 мин эмбеддингов, ~300 МБ векторов в памяти до записи); больше — ошибка задачи с советом разбить файл. Воркер однопоточный: второй документ ждёт первый.
...
- Given в базе нет ни одного администратора, When создаётся профиль, Then он получает `is_admin = true` и членство в General — одной транзакцией
- Given администратор уже есть, When создаётся ещё один профиль, Then `is_admin = false`, членство — только в General; личный департамент не создаётся, и выбрать себе департамент новый пользователь не может (ADR-0016)
...
- Given миграции применены, When проверяется список департаментов, Then в нём ровно один департамент по умолчанию (General); второй отклоняет частичный уникальный индекс
...
- Given пользователь открыл страницу документов, When у него один департамент, Then список скрыт и файл уходит в него; When у него General и ровно один другой, Then предвыбран другой; When других больше одного, Then загрузка недоступна до явного выбора. General никогда не предвыбирается, если есть другой, и подписан «visible to everyone»; зона загрузки всегда называет цель (ADR-0016)
...
- Given пользователь выбирает файл `.doc`, `.xlsx`, `.pptx` или без расширения, When он его загружает, Then загрузка отклоняется сразу — до записи файла и строки в базе — со списком поддерживаемых форматов
- Given PDF без текстового слоя (скан), When воркер его обрабатывает, Then задача `failed` с причиной «no text layer — scanned PDF», а не документ `ready` без фрагментов
- Given PDF с кириллицей во встроенных шрифтах, When он проиндексирован, Then слова в фрагментах целые (не «к алендарных»), и полнотекстовый поиск их находит
- Given таблица с заголовком отчёта над строкой заголовков, датами, формулами, пустыми ячейками и ошибками, When она проиндексирована, Then каждая строка — запись `[лист «…», строка N] Заголовок: значение; …`, даты — `ГГГГ-ММ-ДД`, формулы — вычисленным значением, пустые ячейки и ошибки пропущены, строка отчёта — отдельной записью
- Given таблица на 50 000 строк, When пользователь спрашивает о строке по её номеру, Then эта строка среди найденных, и модель отвечает её значениями с цитатой
- Given таблица больше 100 000 фрагментов, When воркер её обрабатывает, Then задача `failed` с советом разбить файл
- Given DOCX с отслеживаемыми правками и полями, When он проиндексирован, Then в текст попадают вставленные правки, но не удалённые и не коды полей
...
**UC12 — удаление департамента**

- Given администратор, When он удаляет департамент, Then у всех его документов стёрты фрагменты и векторы, документы и сам департамент остались надгробиями, членство и назначения адаптеров удалены, в журнале — `department_deleted` со списком документов
- Given член департамента, в том числе загрузивший все его документы, When он пытается удалить департамент, Then отказ `42501`
- Given General, When администратор пытается его удалить, Then отказ `23001`: в General попадает каждый новый профиль
- Given файл документа лежит и в другом департаменте, When департамент удаляется, Then файл в `blobs/` остаётся; файлы, которые больше никому не нужны, удаляются после COMMIT
- Given департамент удалён, When администратор создаёт новый с тем же именем, Then он создаётся: имя уникально только среди живых
- Given интерфейс, When администратор нажимает Delete, Then подтверждение называет число документов и членов, которые будут затронуты

**UC11 — удаление документа**

- Given пользователь загрузил документ и остаётся членом его департамента, When он удаляет документ, Then фрагменты и векторы документа удалены физически, документ пропал из списка и из поиска, а строка `documents` осталась с `deleted_at` и `deleted_by` — записи журнала о прошлых запросах продолжают на неё указывать
- Given член департамента, не загружавший документ, When он пытается его удалить, Then отказ `42501`; тот же отказ — у не-члена и у вызова без личности, чтобы ответ не подтверждал существование документа
- Given член департамента, When он пытается обойти функцию — `DELETE FROM documents`, `UPDATE documents SET deleted_at`, `UPDATE chunks SET content = ''`, Then отказ по привилегиям, независимо от политик на таблице
- Given администратор не состоит в департаменте, When он удаляет ошибочно загруженный туда документ, Then удаление проходит
- Given документ удаляется, пока воркер считает его эмбеддинги, When воркер доходит до записи, Then он ничего не пишет — стёртый текст не возвращается
- Given тот же файл лежит в другом департаменте, When документ удаляется, Then файл в `blobs/` остаётся; если других ссылок нет — удаляется
- Given документ удалён, When тот же файл загружают снова, Then создаётся новый документ, а не ошибка уникальности
...
| UC11 | Автор документа / Администратор | Удаляет документ |
| UC12 | Администратор | Удаляет департамент вместе с документами |
...
- Main flow:
  ...
  3. Извлечение текста по содержимому файла — PDF, DOCX или текст (ADR-0019), нарезка 512 / 64 символов
...
flowchart LR
  ...
  subgraph Enclave
    ...
    UC11([UC11: Удалить документ])
    UC12([UC12: Удалить департамент])
  end
...
Администратор — тот же сотрудник с флагом `is_admin`, а не отдельный тип учётной записи: он состоит в General и в тех департаментах, куда себя добавил, загружает документы и задаёт вопросы на общих основаниях, и RLS на его запросы действует так же. Отдельны только административные команды. UC7 включается в UC6 асинхронно: загрузка завершается, не дожидаясь приёма (ADR-0010).
...
stateDiagram-v2
  ...
  Ready --> Deleted: delete_document() — автор или администратор
  Pending --> Deleted: задача помечается failed, воркер ничего не пишет
  Failed --> Deleted
  Deleted --> [*]: надгробие; тот же файл можно загрузить как новый документ
  Ready --> [*]
```
...
| `documents` | член | член (WITH CHECK) | член, только `status` и `updated_at` | нет привилегии; удаление — только `delete_document()` |
| `chunks` | член | нет привилегии (пишет воркер) | нет привилегии | нет привилегии |
| `chunk_embeddings` | член | нет привилегии (пишет воркер) | нет привилегии | нет привилегии |
| `department_adapters` | член | нет привилегии | нет привилегии | нет привилегии |
| `ingestion_jobs` | член департамента документа | то же | нет привилегии | нет привилегии |
| `audit_log` | свои строки | свои строки | нет привилегии | нет привилегии |
| `users`, `embedding_models`, `adapters` | все строки (без RLS) | нет привилегии | нет привилегии | нет привилегии |
| `departments`, `department_members` | см. выше | нет привилегии | нет привилегии | нет привилегии |

До миграции 012 у `app_user` было право на запись почти во все таблицы — и в `users` без RLS: `UPDATE users SET is_admin = true` под обычной ролью проходил (проверено, в том числе на чистой базе из миграций). Теперь запись — только в трёх местах, перечисленных в NFR3 (ADR-0018).

«—» — политики нет, при `FORCE` это означает отказ. «Нет привилегии» — сильнее: право отозвано у роли (миграция 009), и никакая разрешающая политика, включая спящие (ADR-0011), его не вернёт. Удаление документа идёт только через `delete_document()` (`SECURITY DEFINER`): она берёт личность из `app.current_user_id` той же транзакции и пропускает автора, пока он член департамента, или администратора (ADR-0015). `is_member_of()` — `SECURITY DEFINER`, чтобы читать `department_members` в обход её собственной политики; `current_user_id()` — plpgsql, возвращает `NULL` на пустой или битой переменной вместо исключения, поэтому политика закрывается, а не падает.
...
7. Документ удаляет только автор или администратор и только через `delete_document()`; прямые пути (`DELETE`, `UPDATE deleted_at`, запись в `chunks`) закрыты привилегиями; после удаления у документа нет ни фрагментов, ни векторов.

Сейчас `rls_validation` покрывает инварианты 2, 3 и 7 и частично 1 (чужой `user_id` → ноль строк; задача приёма без `set_config` не видна). Инварианты 4–6 в тестах не выражены.
...
| --- | --- | --- |
| `hnsw.iterative_scan` при выбранных файлах | `strict_order` | обычный HNSW отбирает `ef_search` = 40 ближайших по всему индексу и фильтрует после: для одного файла среди многих — пусто. Замер на 50 000 фрагментах: выбранный реестр — 6 мс (HNSW), маленький файл — 1,7 мс (планировщик идёт по `idx_chunks_document_id`, миграция 015), точный перебор реестра — 1,4 с |
...
**Редкость слов (IDF) в лексической ветке (ADR-0020).** `ts_rank_cd` в Postgres не учитывает редкость слова: на таблице из 50 000 строк вопрос «какая сумма у договора Д-012345» совпадал со всеми строками по заголовкам («сумма», «договора» — в каждой строке), и нужная строка оказывалась 12 501-й. Теперь частота каждого слова вопроса считается под RLS — по фрагментам, видимым пользователю — с потолком 1 000; кандидаты — только фрагменты с редкими словами, ранг — сумма ln(1 + 1000 / df), `ts_rank_cd` разрешает ничьи. Вопрос без редких слов идёт прежним путём. Результат: нужная строка — в первой двойке, поиск 2,2 → 0,22 с. Закреплено `tests/retrieval_ranking.rs`, который без IDF падает.

**Цена политик RLS на поиске (ADR-0021).** Полнотекстовый GIN-индекс под RLS не используется вовсе: оператор `@@` не leakproof, и планировщик не может применить его раньше политики. Поэтому лексическая ветка проходит по всем фрагментам департамента, и решает цена политики на строку: `is_member_of()` с подзапросом на каждой строке (плюс спящая политика в старых базах) стоила 257 мс на слово по 50 000 фрагментам, массив департаментов, вычисленный один раз, — 15 мс. Поиск остаётся линейным по размеру департамента; на сотнях тысяч фрагментов понадобится решение из альтернатив ADR-0021.

**Таблицы и агрегация.** Поиск находит строки, о которых спрашивают; посчитать по таблице он не может. «Сколько договоров исполнено», «общая сумма по Петровой» не имеют ответа ни в одном фрагменте — модель видит пять строк. Такие вопросы требуют другого механизма (запросов к самой таблице), это отдельное решение.
...
**Первый smoke-прогон (2026-09-23, интерфейс, Qwen 2.5 3B).** Два пользователя: Vlad (Vlad's Department — четыре готовых документа) и Anna (Anna's Department + Sales — `SPEC.md`). Anna задала 7 вопросов, в том числе прямо о документах Vlad. Проверка — не по тексту ответов, а по журналу аудита: все 35 фрагментов, попавших в контекст её вопросов, принадлежат `SPEC.md` из Sales; из Vlad's Department — ноль. Изоляция на уровне ответа подтверждена (NFR2).

Попутно — две проблемы качества, не изоляции. *Цитата может быть подписана неверно:* на вопрос «ARCHITECTURE.md Vlad department, what is in it?» модель ответила «[Source 1] ARCHITECTURE.md does not contain…», хотя `[Source 1]` был `SPEC.md` — имя файла она взяла из вопроса. Список `sources` под ответом строится кодом и верен; текст ответа — нет. Метрика «корректность цитат» из таблицы выше нужна именно поэтому. *Обзорные вопросы* («что в файле SPEC.md») дают размытый ответ: ответа нет ни в одном фрагменте, RAG с top-5 на такие вопросы не рассчитан. Прямые вопросы («как называется приложение») — точны.
...
Источник истины — `enclave/migrations/001–016`. `db/schema.sql` — исходный референсный проект, нигде не применяется и с живой схемой расходится.
...
erDiagram
  ...
  DOCUMENTS ||--o{ SHEET_TABLES : tabulated_as
  SHEET_TABLES ||--o{ SHEET_ROWS : holds
...
| departments | Департаменты. Видны пользователю, только если он в них член (миграция 005). `is_default` — общий департамент General, в который попадает каждый новый профиль; ровно один (миграция 010, ADR-0016). `deleted_at` / `deleted_by` — надгробие удалённого департамента (ADR-0017); имя уникально только среди живых |
| department_members | Членство — то, на чём стоит вся изоляция: `is_member_of()` читает именно эту таблицу |
| documents | Документ без пути к файлу: байты адресуются `file_hash` в блоб-хранилище. Уникальность `(department_id, file_hash)` среди живых документов — один и тот же файл в двух департаментах — два документа, и это правильно: права у них разные. `deleted_at` / `deleted_by` — надгробие удалённого документа (ADR-0015): текста у него больше нет, строка остаётся для журнала аудита |
...
| sheet_tables | Лист таблицы как таблица (ADR-0022): столбцы с выведенным типом (число / дата / текст), частыми значениями и диапазонами — это видит планировщик. Пишет только воркер, в транзакции приёма; удаляется вместе с документом триггером на `documents.deleted_at` |
| sheet_rows | Строки листа: `cells` — JSONB-массив по столбцам, числа как JSON-числа (Postgres читает их как `numeric` — суммы точные). `department_id` денормализован, как у фрагментов |
...
| partial unique `documents (department_id, file_hash) WHERE deleted_at IS NULL` | дедупликация в пределах департамента; удалённый файл можно загрузить заново (миграция 009) |
| unique `department_adapters (department_id) WHERE is_default` | один адаптер по умолчанию |
| PK `sheet_rows (table_id, row_number)` | расчёт читает строки одной таблицы (миграция 014) |

**Спящие объекты.** В базах, которые когда-то были созданы вручную из `db/schema.sql`, оставались таблицы `memberships` / `roles` / `conversations` / `query_logs` и двенадцать разрешающих политик на функциях `app_user_in_department()` / `app_current_user_id()`. Миграция 013 удалила политики и функции (ADR-0021, замещает ADR-0011): до и после сняты отпечатки видимого каждому пользователю по восьми таблицам — совпали полностью. Сами таблицы остались, с RLS и без политик, то есть закрыты полностью.
...
| `cmd_list_documents` | app | да | Живые документы своих департаментов; `can_delete` — подсказка интерфейсу, решает база |
| `cmd_delete_document {document_id}` | app | да | UC11 через `delete_document()`; файл блоба удаляется после COMMIT, если на хэш больше никто не ссылается |
...
| Команда | Пул / роль | Контекст RLS | Назначение |
| --- | --- | --- | --- |
| `cmd_list_document_tables` | app | да | FR21 → `[{document_id, sheet, row_count, columns[{name, type}]}]` по таблицам, видимым под RLS; частые значения и диапазоны, которые видит планировщик, не отдаются |
| `cmd_query {query, top_k?, plan?, document_ids?}` | app | да | UC9 без стриминга → `{answer, sources[], calculation, clarification}`; `calculation` — текст расчёта по таблице или `null` (ADR-0022); `clarification` — `{question, table_id, options[{label, plan}]}` вместо ответа; `plan` — `{table_id, plan}` выбранного варианта, проверяется как вывод модели (ADR-0023); `document_ids` — выбранные в панели файлы, поиск и расчёты только по ним (FR22) |
| `cmd_query_stream {request_id, args: {query, top_k?}}` | app | да | UC9 со стримингом: токены в событии `llm-token:<request_id>` как `{token}`, итог — `{answer, sources[], calculation}` |
...
| Команда | Назначение |
| --- | --- |
| `cmd_list_departments` | Все живые департаменты с числом членов и документов |
| `cmd_create_department {name}` | UC3 |
| `cmd_delete_department {department_id}` | UC12 через `delete_department()` на `app_pool` с личностью из сессии — проверку делает база, а не `require_admin` |
...
**ADR-0011. Отложенная чистка спящих RLS-политик на `memberships`.** *Замещено ADR-0021.* Миграция 005 не трогает семь разрешающих политик, оставшихся от ручного применения `schema.sql`; снос — отдельной миграцией, по таблице за раз, с прогоном `rls_validation` после каждой. Обоснование: радиус поражения больше, чем у миграции выравнивания колонок. Trade-off: в dev-базе живёт мина, безопасная только пока `memberships` пуста.

**ADR-0014. PostgreSQL поставляется вместе с приложением.** Урезанный PostgreSQL 18 (сборка EDB) и pgvector 0.8.3, собранный из исходников MSVC-компилятором: 61 МБ, 24,5 МБ в архиве, состав по зависимостям DLL и по миграциям. Первый запуск — `initdb` (11–13 с) с локалью `builtin C.UTF-8` (регистр кириллицы без локалей Windows), `scram-sha-256`, только `127.0.0.1`, свободный порт. `postgres.exe` запускается приложением без консоли: через `pg_ctl` из консоли сервер падал с `0xC0000142`, когда консоль закрывалась. Остановка — `pg_ctl stop -m fast`. Пароли генерируются и хранятся под DPAPI. Прототип прошёл все интеграционные тесты без Docker. Отклонены: Docker, установленный пользователем PostgreSQL, системная служба, SQLite/встраиваемые векторные базы (нет RLS), PGlite, сторонние сборки pgvector. Trade-off: +24,5 МБ к установщику, обновления мажорных версий и резервные копии — забота приложения.

**ADR-0015. Удаление документов.** Удаляет автор (пока член департамента) или администратор. Текст — фрагменты и векторы — стирается физически, строка `documents` остаётся надгробием, потому что журнал аудита ссылается на документы по id. Правило живёт в базе: у `app_user` отозваны `DELETE` и `UPDATE deleted_at` на `documents` и любая запись в `chunks`, единственный путь — `delete_document()`. Trade-off: soft-delete сохранил бы больше для аудита, но оставил бы текст ошибочно загруженного документа в базе; физическое удаление строки сломало бы журнал. Надгробия копятся — их срок хранения открыт вместе с retention журнала.

**ADR-0016. Общий департамент по умолчанию, членство назначает администратор.** Новый профиль попадает только в General; личные департаменты больше не создаются; остальное членство назначает администратор. Выбирать департамент при создании профиля пользователю не дано: департамент — это право доступа, а профиль создаётся с экрана входа без аутентификации, так что выбор отдал бы границу изоляции тому, кого она ограничивает. Отложено, а не отклонено: запрос департамента с подтверждением администратором. Trade-off: всё в General видит каждый, поэтому General как цель загрузки выбирается только явно — другие департаменты можно предвыбрать.

**ADR-0017. Удаление департамента вместе с документами.** Только администратор, General — никогда. Документы удаляются как в ADR-0015, членство и назначения адаптеров — тоже, департамент остаётся надгробием: на него ссылаются `RESTRICT`-ключи, надгробия документов и журнал. Проверка — функция `delete_department()` в базе. Отклонены: «только пустой департамент» (личные департаменты с документами пришлось бы разбирать по одному), перенос документов (смена департамента — новое решение о доступе к каждому документу). Trade-off: одна кнопка стирает содержимое целого департамента; защита — подтверждение с числом документов и членов.

**ADR-0018. Минимальные привилегии `app_user`.** Роль пользовательских запросов пишет ровно то, что пишет приложение: вставка в `documents` (и `status` / `updated_at`), `ingestion_jobs`, `audit_log`. Всё остальное — только чтение; привилегированное — через `admin_pool` или функции `SECURITY DEFINER`. Найдено при закрытии прав на адаптеры: `app_user` мог назначить себя администратором одной строкой SQL, а привилегии по умолчанию открывали бы на запись каждую новую таблицу. Отклонено: RLS на `users` вместо привилегий (политика «своя строка» всё равно пропустила бы `is_admin` у своей строки). Trade-off: каждая новая запись через `app_pool` требует явного `GRANT` в миграции — намеренное трение.

**ADR-0019. Извлечение текста — PDF и DOCX на чистом Rust, без OCR.** Формат — по содержимому, а не по MIME; неподдерживаемое отклоняется при загрузке. PDF — парсер `pdf-extract`, но своя сборка текста: штатный вывод разрывал слова (замерено: ложный разрыв внутри слова ~1 em, шире пробела), поэтому слова разделяются настоящими пробелами документа, а геометрия используется, только если их нет. DOCX — свой разбор `zip` + `quick-xml`. Пустой результат — ошибка, сканы отклоняются с причиной. Отклонены: pdfium / poppler (нативная библиотека на каждую ОС до решения ADR-0014), подбор порога разрыва (перекрываются ложные разрывы и узкие колонки таблиц). Trade-off: ячейка таблицы, заполненная до отступа, сливается со следующей.

**ADR-0020. Таблицы и поиск на их масштабе.** Строка таблицы — самостоятельная запись с листом, номером строки и заголовками; фрагменты собираются из целых строк. Три узких места, найденные замером на 50 000 строках: эмбеддинги (пакеты по 32 и тихий лог llama-server: 49 → 148 текстов/с), запись (пакетные вставки: 11 → 3,9 мин, остаток — HNSW), ранжирование (IDF по частоте слов под RLS: нужная строка с 12 501-го места — в первую двойку). Отклонены: CSV-выгрузка (заголовки только в первом фрагменте), несколько строк на фрагмент (вектор размывается), перестройка HNSW при загрузке (индекс общий). Trade-off: RAG находит строки, но не агрегирует; ~12 КБ в базе на строку.

**ADR-0021. Политики RLS вычисляют членство один раз на запрос.** `department_id = ANY ((SELECT current_user_department_ids())::uuid[])` вместо функции на каждой строке: поиск 2,2 → 0,22 с при тех же правилах. Спящие политики из `schema.sql` удалены — отпечатки видимости до и после совпали. Отклонены: пометить `@@` как leakproof (правка системного каталога), функция поиска в обход RLS (граница уходит из политик в код).

**ADR-0023. Угаданный столбец — вопрос пользователю; список групп пишет ядро.** Ядро детерминированно проверяет, называет ли вопрос столбец разбивки или метрики (имя или значение, с поправкой на окончания); если нет — вопрос с кнопками, каждая несёт готовый план, который проверяется как вывод модели (таблица по id под RLS). Сгруппированный ответ — список, написанный ядром: модель склеивала группы. Отклонены: третий исход «уточнить» у модели (не замечает своих догадок), подгонка промпта, словарь синонимов. Trade-off: лишний клик там, где догадка была верной.

**ADR-0022. Расчёты по таблицам: план от модели, SQL от ядра.** Каждый лист хранится ещё и как таблица (`sheet_tables` / `sheet_rows`, под RLS, пишет только воркер). Если поиск нашёл строки таблицы, модель строит план — метрика, группировка, фильтры — в границах JSON-схемы, собранной из столбцов этой таблицы (а для столбцов с немногими значениями — из самих значений); ядро проверяет план и выполняет параметризованный агрегат на `app_pool`. Отказ — обычный RAG, а не приблизительное число. Отклонены: text-to-SQL (проверка произвольного SQL — собственный анализатор, каждая дыра в котором — дыра безопасности), отдельная таблица Postgres на лист (DDL из воркера, политики на динамические объекты), расчёт в Rust по выгруженным строкам (без RLS на расчёте). Trade-off: модель может неверно понять вопрос, но не посчитать — поэтому расчёт всегда показан пользователю; +1 вызов модели (~1 с) на вопрос по таблице.
...
| Unit: таблицы | строки-записи с листом, номером и заголовками; заголовок отчёта над заголовками; типы ячеек (дата, формула, логическое, ошибка, пустая); лист без заголовков; распознавание по содержимому; нарезка по целым строкам | `cargo test --lib` | есть |
| Unit: расчёты по таблицам | типы столбцов по большинству ячеек, числа и даты, набранные текстом; схема плана по столбцам таблицы, закрытые столбцы; отказ от планов, не подходящих таблице; параметризация SQL; пара равенств как диапазон; уточнение при неназванном столбце, варианты разбираются обратно в свои планы | `cargo test --lib tables` | есть |
| Integration: расчёты | точные суммы (десятичные), счёт, группы, диапазоны дат, числа сравниваются как числа; чужой департамент не видит ни таблицы, ни строк, в том числе по id таблицы; удаление документа удаляет таблицы | `tests/table_aggregation.rs`; падает без политики RLS на `sheet_rows` и без триггера (проверено) | есть |
| Integration: ранжирование | 1 500 строк с общими словами и одна с нужным номером; поиск приложения под RLS находит её, чужой департамент — ничего; выбранные файлы: поиск только в них, чужой документ по id — ничего | `tests/retrieval_ranking.rs`; без IDF падает (проверено) | есть |
| Integration: контракт схемы | запросы, которые приложение выполняет при старте (регистрация модели эмбеддингов), на базе, собранной только из миграций | `tests/schema_contract.rs`; падает без миграции 016 (проверено) — dev-база, собранная когда-то вручную, это скрывала | есть |
| Integration: изоляция | инварианты 1–7 из «Матрицы доступа» на одноразовой базе | `rls_validation` проходит; тест падает, если снять политику с любой таблицы | есть, покрывает 2–3, 7 и часть 1 |
...
**CI** (`.github/workflows/ci.yml`, на каждый push в `main` и pull request). Две задачи: (1) сборка и все Rust-тесты, включая `rls_validation`, на сервисном контейнере `pgvector/pgvector:pg16` — компилятор зафиксирован в самом workflow, а не в репозитории, чтобы локальная сборка не зависела от скачивания точной версии; (2) проверка типов и сборка фронтенда (`tsc && vite build`). Тест без `TEST_ADMIN_URL` и `TEST_APP_URL` молча пропускается — поэтому CI выставляет `ENCLAVE_REQUIRE_DB_TESTS`, и отсутствие переменных превращает пропуск в падение. На чистом кластере CI роль `app_user` не существует до миграции 004, поэтому тест создаёт её сам — иначе воспроизведение дрейфа привилегий по умолчанию (раздел 9) не сработало бы и проверка «таблица, созданная позже» прошла бы тривиально.
...
| 11 | ~~Любой член департамента может напрямую удалить или изменить назначения LoRA-адаптеров своего департамента~~ | `migrations/012` | **Исправлено**: у `app_user` нет прав на запись в `department_adapters`; политики на запись удалены |
| 12 | ~~`app_user` мог писать почти во все таблицы, включая `users` без RLS: `UPDATE users SET is_admin = true` — эскалация до администратора~~ | `migrations/012` | **Исправлено** (ADR-0018): запись только в `documents`, `ingestion_jobs`, `audit_log`; привилегии по умолчанию на запись и `EXECUTE` сняты; закреплено разделом 9 `rls_validation`, который без миграции падает именно на эскалации |
...
- ~~Smoke-сценарий в интерфейсе~~ — пройден 2026-09-23 (см. «Методика оценки», «Первый smoke-прогон»)
- Smoke-сценарий из CLAUDE.md на настоящей модели; замер NFR4
- ~~CI с Postgres + pgvector и `rls_validation`~~ — сделано; инварианты 4–6 в тесте — нет
- ADR-0012, 0013; ~~ADR-0014~~ — решено и встроено: своя база по умолчанию (`db/embedded.rs`), скрипт сборки комплекта, пароли под DPAPI, CI на PostgreSQL 16 и 18; впереди — включение в установщик и проверка на чистой машине

**v1.1**

- ~~Извлечение текста из PDF и DOCX~~ — сделано (ADR-0019); OCR для сканов — нет
- ~~Расчёты по таблицам~~ — сделано (ADR-0022); несколько метрик, соединение таблиц, вычисляемые выражения — нет
...
| --- | --- | --- | --- |
| Поставка Postgres + pgvector в десктопном установщике | Средняя — решено и проверено прототипом (ADR-0014), не проверено на чистой машине | Высокое — без этого нет дистрибутива | ADR-0014: свой урезанный PostgreSQL 18 и своя сборка pgvector; до релиза — проверка на чистой машине (Visual C++ runtime, антивирус, не-ASCII путь); режим «существующий сервер» как запасной путь |
...
| Роль пользовательских запросов может больше, чем нужно приложению, — ошибка в коде становится эскалацией | Низкая после миграции 012 — **случилось**: `app_user` мог выставить себе `is_admin` | Критическое | Минимальные привилегии (ADR-0018); раздел 9 `rls_validation` перечисляет запрещённые записи и падает при их возвращении; привилегии по умолчанию не дают записи новым таблицам |
