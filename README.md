...
[docs/adr](docs/adr/README.md), шестнадцать принятых Architecture Decision
...
```
docs/adr/              шестнадцать ADR — источник истины по «почему»
...
**Форматы (ADR-0019).** PDF с текстовым слоем, DOCX, TXT и MD; формат
определяется по содержимому файла, а не по типу, который прислал браузер.
Остальное (`.doc`, `.xlsx`, `.pptx`) отклоняется сразу при загрузке, скан без
