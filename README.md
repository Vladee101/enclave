...
[docs/adr](docs/adr/README.md), восемнадцать принятых Architecture Decision
...
Миграции (`enclave/migrations/001-013`) применяются автоматически при каждом
...
```
docs/adr/              восемнадцать ADR — источник истины по «почему»
...
enclave/                                  само приложение
  ...
  src-tauri/src/
    ...
    ingest/extract.rs    текст из PDF / DOCX / TXT / MD (ADR-0019)
...
на запрос (ADR-0021). Чего не умеет: считать по таблице — «сколько договоров
исполнено» или «общая сумма» не лежат ни в одной строке. Предел — 100 000
фрагментов на документ; ~12 КБ в базе на строку. Что найдёт поиск и что
ответит модель, можно проверить без интерфейса:
...
строит код. Не проверены LoRA-адаптеры (их пока нет ни одного) и PDF / DOCX —
извлечение текста по-прежнему только для `.txt` / `.md`.
