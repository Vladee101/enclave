...
Миграции (`enclave/migrations/001-015`) применяются автоматически при каждом
...
| 9 | Поставка PostgreSQL + pgvector вместе с приложением | решено (ADR-0014), прототип проверен: PostgreSQL 18 + свой pgvector без Docker, 61 МБ, все тесты зелёные; в приложение не встроено |
...
решена (ADR-0014): приложение будет само поставлять урезанный PostgreSQL 18 со
своей сборкой pgvector и само его запускать. Прототип проверен на Windows без
Docker, в приложение он пока не встроен.
