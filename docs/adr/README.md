...
| [0014](0014-postgresql-distribution.md) | PostgreSQL поставляется вместе с приложением и запускается им самим | Принято |
...
| [0023](0023-asking-back-and-core-written-groups.md) | Расчёт по угаданному столбцу — вопрос пользователю; список групп пишет ядро | Принято |
| [0024](0024-models-downloaded-on-first-run.md) | Модели скачиваются при первом запуске; чат-модель — Qwen3-4B | Принято |
| [0025](0025-inference-engine-per-machine.md) | Движок llama.cpp скачивается при первом запуске под видеокарту машины | Принято |

Номера 0012 и 0013 зарезервированы за решениями из раздела ниже.

## Не записанные решения

Два решения из раздела «Open decisions» в [CLAUDE.md](../../CLAUDE.md) ещё
ждут своего ADR: граница доверия воркера загрузки (0012) и
контентно-адресуемое хранилище блобов (0013). Оба уже реализованы в коде,
записать нужно обоснование.
